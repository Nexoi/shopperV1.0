...
import com.seeu.shopper.ashop.dao.ProductSearchV2Mapper;
...
//    ProductSearchMapper productSearchMapper;
    @Resource
    ProductSearchV2Mapper productSearchV2Mapper;
    ...
    public List<ProductSearchResultModel> searchPros(Integer fatherCateID, String keyword, Integer sortRule, boolean priceCondition, BigDecimal priceLow, BigDecimal priceHigh) {
        ...
        if (priceCondition) {
            ...
            priceCondi = "AND ( current_price > " + priceLow.doubleValue() + " AND current_price < " + priceHigh.doubleValue() + ")";
        }
        ...
        switch (sortRule) {
            case -1:
                break;
            case 1:
                sortCondi = "order by current_price";
                break;
            case 2:
                sortCondi = "order by current_price desc";
                break;
            case 3:
                sortCondi = "order by sales";
                break;
            case 4:
                sortCondi = "order by sales desc";
                break;
            default:
                break;
        }
        ...
        if (fatherCateID == null) {
            // 查询所有
            if (keyword == null) {
                if (priceCondi.length() != 0) {
                    return productSearchV2Mapper.listAllWithPriceCondition(priceCondi, sortCondi);
                }
                return productSearchV2Mapper.listAll(sortCondi);
            }
            return productSearchV2Mapper.searchAll(keyword.trim(), priceCondi, sortCondi);
        } else {
...
//                return searchPros(null, keyword, sortRule, priceCondition, priceLow, priceHigh);
            ...
            if (keyword == null) {
                return productSearchV2Mapper.listCates(sqlchip, priceCondi, sortCondi);
            } else {
                return productSearchV2Mapper.searchCates(keyword, sqlchip, priceCondi, sortCondi);
            }
